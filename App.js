...
const themes = {
  light: {
    dark: false, background: '#f8f8f8', card: '#ffffff', text: '#333', textSecondary: '#666', border: '#eee', primary: '#3CB371', positive: '#28a745', negative: '#E53E3E',
    tabInactive: '#666',
    tabActiveText: '#fff',
  },
  dark: {
    dark: true, background: '#121212', card: '#1e1e1e', text: '#f5f5f5', textSecondary: '#a0a0a0', border: '#2c2c2c', primary: '#3CB371', positive: '#28a745', negative: '#E53E3E',
    tabInactive: '#a0a0a0',
    tabActiveText: '#fff',
  },
};
...
const ThemeProvider = ({ children }) => {
  ...
  useEffect(() => { const loadTheme = async () => { const savedTheme = await AsyncStorage.getItem('theme'); if (savedTheme) { setTheme(savedTheme); } }; loadTheme(); }, []);
  const toggleTheme = async () => { const newTheme = theme === 'light' ? 'dark' : 'light'; setTheme(newTheme); await AsyncStorage.setItem('theme', newTheme); };
...
// --- SERVIÇO DA API ---
const apiService = {
  deleteQuest: async (questId, token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Quest/deletar?questId=${questId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Erro ao deletar missão (Status: ${response.status}): ${errorText}`); } return { success: true }; },
  getTransactions: async (token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Transacao/listar-transacao`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Erro ao buscar transações (Status: ${response.status}): ${errorText}`); } return response.json(); },
  getUserProfile: async (token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Usuario/buscar-usuario`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Erro ao buscar perfil do usuário (Status: ${response.status}): ${errorText}`); } return response.json(); },
  getQuestsByStatus: async (token, status) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Quest/buscar-quests?status=${status}`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) { console.error(`Erro ao buscar missões com status ${status}: ${response.status}`); return []; } const quests = await response.json(); return Array.isArray(quests) ? quests : []; },
  createQuest: async (questData, token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Quest/criar-quest-manual`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(questData) }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Erro ao criar missão (Status: ${response.status}): ${errorText}`); } return { success: true }; },
  firebaseLogin: async (email, password) => { const response = await fetch(`${FIREBASE_AUTH_BASE_URL}:signInWithPassword?key=${FIREBASE_API_KEY}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, returnSecureToken: true }), }); const data = await response.json(); if (!response.ok) throw new Error(data.error.message); return data; },
  coinUpLogin: async (email, password) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Auth/login?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`, { method: 'GET' }); const responseText = await response.text(); let data; try { data = JSON.parse(responseText); } catch (e) { throw new Error(`Erro de comunicação com o servidor: ${responseText}`); } if (!response.ok || data.statusCode !== 200) { throw new Error(data.message || 'Email ou senha inválidos na API CoinUp.'); } return data; },
  createUserProfile: async (profileData) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Usuario/cadastro`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(profileData) }); if (!response.ok) { const errorText = await response.text(); try { const errorJson = JSON.parse(errorText); throw new Error(errorJson.message || `Erro ao criar perfil: ${errorText}`); } catch (e) { throw new Error(`Erro ao criar perfil de usuário: ${errorText}`); } } return { success: true }; },
  getDashboardData: async (token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Conta/tela-home`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}` } }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Não foi possível carregar os dados do dashboard (Status: ${response.status}): ${errorText}`); } return response.json(); },
  createTransaction: async (transactionData, token) => { const response = await fetch(`${COINUP_API_BASE_URL}/api/app/Transacao/criar-transacao`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(transactionData) }); if (!response.ok) { const errorText = await response.text(); throw new Error(`Erro ao criar transação: ${errorText}`); } return { success: true }; },
};

// --- Componentes de UI ---
const ProfileIcon = ({ onPress }) => { const { theme } = useTheme(); const styles = getStyles(theme); return ( <TouchableOpacity onPress={onPress} style={styles.iconButton}><View style={[styles.iconCircle, {backgroundColor: theme.border}]}><View style={[styles.iconShape, styles.profileHead, {backgroundColor: theme.textSecondary}]} /><View style={[styles.iconShape, styles.profileBody, {backgroundColor: theme.textSecondary}]} /></View></TouchableOpacity> ); };
const BackIcon = ({ onPress }) => { const { theme } = useTheme(); const styles = getStyles(theme); return ( <TouchableOpacity onPress={onPress} style={styles.backButton}><Text style={[styles.backButtonText, {color: theme.primary}]}>‹</Text></TouchableOpacity> ); };
const PasswordVisibilityToggle = ({ isVisible, onPress }) => { const { theme } = useTheme(); const styles = getStyles(theme); return ( <TouchableOpacity onPress={onPress} style={styles.passwordVisibilityButton}><Text style={[styles.passwordVisibilityText, {color: theme.primary}]}>{isVisible ? 'Ocultar' : 'Ver'}</Text></TouchableOpacity> ); };
const CalendarModal = ({ visible, onClose, onSelectDate }) => { const { theme } = useTheme(); const styles = getStyles(theme); const [date, setDate] = useState(new Date()); const getMonthDays = (year, month) => { const firstDay = new Date(year, month, 1).getDay(); const daysInMonth = new Date(year, month + 1, 0).getDate(); let days = Array(firstDay).fill(null); for (let i = 1; i <= daysInMonth; i++) days.push(i); return days; }; const days = getMonthDays(date.getFullYear(), date.getMonth()); const weekDays = ['D', 'S', 'T', 'Q', 'Q', 'S', 'S']; const monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]; return ( <RNModal transparent={true} animationType="fade" visible={visible} onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalContent}><Text style={styles.modalTitle}>{monthNames[date.getMonth()]} {date.getFullYear()}</Text><View style={styles.calendarGrid}>{weekDays.map((day, index) => <Text key={index} style={styles.calendarWeekDay}>{day}</Text>)}{days.map((day, index) => ( <TouchableOpacity key={index} style={styles.calendarDay} onPress={() => day && onSelectDate(new Date(date.getFullYear(), date.getMonth(), day))}><Text style={[styles.calendarDayText, day ? {} : {color: theme.card}]}>{day}</Text></TouchableOpacity> ))}</View><TouchableOpacity style={styles.modalCloseButton} onPress={onClose}><Text style={styles.modalCloseButtonText}>Fechar</Text></TouchableOpacity></View></View></RNModal> ); };
const SelectionModal = ({ visible, title, options, onSelect, onClose }) => { const { theme } = useTheme(); const styles = getStyles(theme); if (!visible) return null; return ( <RNModal transparent={true} animationType="fade" visible={visible} onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalContent}><Text style={styles.modalTitle}>{title}</Text><ScrollView>{options.map((option, index) => ( <TouchableOpacity key={index} style={styles.modalOption} onPress={() => onSelect(option)}><Text style={styles.modalOptionText}>{option}</Text></TouchableOpacity> ))}</ScrollView><TouchableOpacity style={styles.modalCloseButton} onPress={onClose}><Text style={styles.modalCloseButtonText}>Fechar</Text></TouchableOpacity></View></View></RNModal> ); };
const AlertModal = ({ visible, title, message, onClose }) => { const { theme } = useTheme(); const styles = getStyles(theme); return ( <RNModal transparent visible={visible} animationType="fade" onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalContent}><Text style={styles.modalTitle}>{title}</Text><Text style={styles.alertMessage}>{message}</Text><TouchableOpacity style={styles.modalCloseButton} onPress={onClose}><Text style={styles.modalCloseButtonText}>OK</Text></TouchableOpacity></View></View></RNModal> ); };
const AnimatedProgressBar = ({ progress, color }) => { const { theme } = useTheme(); const styles = getStyles(theme); const animation = useRef(new Animated.Value(0)).current; useEffect(() => { Animated.timing(animation, { toValue: progress, duration: 500, useNativeDriver: false }).start(); }, [progress]); const width = animation.interpolate({ inputRange: [0, 100], outputRange: ["0%", "100%"], extrapolate: "clamp" }); return (<View style={styles.progressBarBackground}><Animated.View style={[styles.progressBarFill, { width, backgroundColor: color || theme.primary }]} /></View>); };
const AnimatedNumber = ({ value, suffix = '', style }) => { const animation = useRef(new Animated.Value(value || 0)).current; const [displayValue, setDisplayValue] = useState(value || 0); useEffect(() => { const listener = animation.addListener(({ value }) => setDisplayValue(value)); Animated.timing(animation, { toValue: value || 0, duration: 300, useNativeDriver: false }).start(); return () => animation.removeListener(listener); }, [value]); return <Animated.Text style={style}>{displayValue.toFixed(0)}{suffix}</Animated.Text>; };
...
const LoginScreen = ({ onNavigate, onLoginSuccess }) => {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
      <AlertModal visible={!!error} title="Erro de Login" message={error} onClose={() => setError(null)} />
      <View style={styles.content}>
        <Text style={[styles.title, {marginBottom: 60}]}>
          Bem vindo ao <Text style={styles.titleHighlight}>CoinUP</Text>
        </Text>
        <Text style={styles.subtitle}>Faça login para gerenciar sua carteira</Text>
        <View style={styles.inputContainer}><TextInput style={styles.inputField} placeholder="Email" placeholderTextColor={theme.textSecondary} keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail} /></View>
        <View style={styles.inputContainer}><TextInput style={styles.inputField} placeholder="Senha" placeholderTextColor={theme.textSecondary} secureTextEntry={!isPasswordVisible} value={password} onChangeText={setPassword} /><PasswordVisibilityToggle isVisible={isPasswordVisible} onPress={() => setIsPasswordVisible(!isPasswordVisible)} /></View>
        ...
        <TouchableOpacity style={styles.linkButton} onPress={() => onNavigate('signup')}><Text style={styles.linkText}>Não tem uma conta? <Text style={styles.linkTextBold}>Cadastre-se</Text></Text></TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
};
...
const HomeScreen = ({ onNavigate, missions, dashboardData, isLoading, dashboardError, isRefreshing, onRefresh }) => {
    ...
    if (isLoading) { return ( <View style={[styles.centeredMessageContainer, {backgroundColor: theme.background}]}><ActivityIndicator size="large" color={theme.primary} /></View> ); }
    if (dashboardError) { return ( <View style={[styles.centeredMessageContainer, {backgroundColor: theme.background}]}><Text style={styles.errorMessage}>{dashboardError}</Text></View> ); }
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                <TouchableOpacity onPress={() => onNavigate('history')}><View style={styles.balanceCard}><Text style={styles.balanceCardTitle}>Saldo Atual</Text><Text style={styles.balanceCardAmount}>R$ {conta?.saldoAtual?.toFixed(2).replace('.', ',') || '0,00'}</Text></View></TouchableOpacity>
                <View style={styles.summaryContainer}><TouchableOpacity style={[styles.summaryCard, { backgroundColor: theme.dark ? 'rgba(46, 139, 87, 0.2)' : '#E6F5EC' }]} onPress={() => onNavigate('addIncome')}><Text style={styles.summaryCardTitle}>Receitas</Text><Text style={[styles.summaryCardAmount, { color: theme.positive }]}>R$ {receita?.toFixed(2).replace('.', ',') || '0,00'}</Text></TouchableOpacity><TouchableOpacity style={[styles.summaryCard, { backgroundColor: theme.dark ? 'rgba(229, 62, 62, 0.2)' : '#FEECEE' }]} onPress={() => onNavigate('addExpense')}><Text style={styles.summaryCardTitle}>Despesas</Text><Text style={[styles.summaryCardAmount, { color: theme.negative }]}>R$ {despesa?.toFixed(2).replace('.', ',') || '0,00'}</Text></TouchableOpacity></View>
                <TouchableOpacity style={styles.missionsCard} onPress={() => onNavigate('missions')}><View style={styles.missionsCardHeader}><Text style={styles.missionsCardTitle}>Missões Ativas</Text><Text style={styles.missionsViewMore}>Ver mais &gt;</Text></View><View style={styles.separator} />{missions && missions.length > 0 ? missions.slice(0, 4).map((mission, index) => { const progress = mission.progressoAtual || 0; return ( <View key={mission.id} style={[styles.missionItem, index === missions.slice(0, 4).length - 1 ? { marginBottom: 0 } : {}]}><View><Text style={{color: theme.text}}>{mission.descricao}</Text><Text style={styles.missionXpText}>+ {mission.pontosDeExperiencia} XP</Text></View><View style={styles.missionProgressContainer}><Text style={styles.missionProgress}>Progresso: </Text><AnimatedNumber value={progress} suffix="%" style={styles.missionProgress} /></View></View> );}) : <Text style={{paddingVertical: 10, color: theme.textSecondary}}>Nenhuma missão ativa.</Text>}</TouchableOpacity>
            </ScrollView>
        </View>
    );
};
...
const App = () => {
  ...
  const loadAllData = async (token) => {
    ...
    try {
        ...
        setDashboardError("Não foi possível carregar os dados principais."); 
        setIsLoading(false); 
        return; 
    }
    
    try {
        const [ativas, concluidas, falhadas] = await Promise.all([
        ...
        setAllMissions({
            ...
            falhas: falhadas.sort((a,b) => new Date(b.dataDeCriacao) - new Date(a.dataDeCriacao))
        });
    } catch(error) { 
        console.error("ERRO AO CARREGAR MISSÕES:", error);
        setActiveMissions([]);
        setAllMissions({ ativas: [], concluidas: [], falhas: [] });
    }
  ...
  return ( 
    <View style={styles.container}>
      ...
      <AlertModal visible={alertInfo.visible} title={alertInfo.title} message={alertInfo.message} onClose={() => setAlertInfo({ ...alertInfo, visible: false })} />
      {renderScreen()}
    </View> 
  );
}
...
// ===== ESTILOS DINÂMICOS ======================================
// ==============================================================
const getHistoryStyles = (theme) => StyleSheet.create({ container: { flex: 1, backgroundColor: theme.dark ? '#000' : '#f4f6f8' }, header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight + 10 : 50, paddingBottom: 15, paddingHorizontal: 16, backgroundColor: theme.dark ? '#000' : '#f4f6f8' }, headerTitle: { fontSize: 20, fontWeight: 'bold', color: theme.text }, backButton: { position: 'absolute', left: 10, top: Platform.OS === 'android' ? StatusBar.currentHeight + 5 : 45, width: 40, height: 40, justifyContent: 'center', alignItems: 'center', zIndex: 1 }, backButtonText: { fontSize: 32, fontWeight: 'bold', color: theme.text }, listContent: { paddingHorizontal: 16, paddingBottom: 20 }, sectionHeader: { fontSize: 15, fontWeight: '600', color: theme.textSecondary, paddingTop: 24, paddingBottom: 8, backgroundColor: theme.dark ? '#000' : '#f4f6f8' }, card: { backgroundColor: theme.card, borderRadius: 8, paddingVertical: 14, paddingHorizontal: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, borderWidth: theme.dark ? 1 : 0, borderColor: theme.border }, cardDetails: { flex: 1, marginRight: 10 }, cardTitle: { fontSize: 16, fontWeight: '500', color: theme.text }, cardDescription: { fontSize: 14, color: theme.textSecondary, marginTop: 3 }, cardAmount: { fontSize: 16, fontWeight: 'bold' }, emptyText: { textAlign: 'center', marginTop: 50, fontSize: 16, color: theme.textSecondary } });
const getStyles = (theme) => StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.background }, keyboardView: { flex: 1 }, content: { flexGrow: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 30, paddingVertical: 20 },
  title: { fontSize: 26, fontWeight: 'bold', color: theme.text, marginBottom: 10, textAlign: 'center' },
  titleHighlight: { fontSize: 30, fontWeight: 'bold', color: theme.primary },
  subtitle: { fontSize: 16, color: theme.textSecondary, marginBottom: 40, textAlign: 'center' }, inputContainer: { width: '100%', height: 50, backgroundColor: theme.card, borderRadius: 12, marginBottom: 15, borderWidth: 1, borderColor: theme.border, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }, inputField: { flex: 1, height: '100%', fontSize: 16, color: theme.text }, passwordVisibilityButton: { padding: 5 }, passwordVisibilityText: { fontWeight: 'bold' }, genderContainer: { width: '100%', flexDirection: 'row', alignItems: 'center', marginBottom: 15, justifyContent: 'center' }, genderLabel: { fontSize: 16, color: theme.textSecondary, marginRight: 10 }, genderButton: { borderWidth: 1, borderColor: theme.border, borderRadius: 12, paddingVertical: 10, paddingHorizontal: 20, marginRight: 10, backgroundColor: theme.card }, genderButtonSelected: { backgroundColor: theme.primary, borderColor: theme.primary }, genderButtonText: { fontSize: 14, color: theme.text }, genderButtonTextSelected: { color: '#fff', fontWeight: 'bold' }, button: { width: '100%', height: 50, backgroundColor: theme.primary, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginTop: 10 }, buttonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' }, linkButton: { marginTop: 25 }, linkText: { fontSize: 14, color: theme.textSecondary }, linkTextBold: { fontWeight: 'bold', color: theme.primary }, homeContainer: { flex: 1, padding: 20, backgroundColor: theme.background }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }, pageTitle: { fontSize: 20, fontWeight: 'bold', color: theme.text }, headerIcons: { flexDirection: 'row' }, iconButton: { marginLeft: 10, padding: 5 }, iconCircle: { width: 32, height: 32, borderRadius: 16, justifyContent: 'flex-end', alignItems: 'center', overflow: 'hidden' }, iconShape: {}, profileHead: { width: 12, height: 12, borderRadius: 6, marginBottom: -2 }, profileBody: { width: 24, height: 12, borderTopLeftRadius: 12, borderTopRightRadius: 12 }, balanceCard: { backgroundColor: theme.primary, borderRadius: 16, padding: 25, alignItems: 'center', marginBottom: 20 }, balanceCardTitle: { color: '#fff', fontSize: 16 }, balanceCardAmount: { color: '#fff', fontSize: 32, fontWeight: 'bold', marginTop: 5 }, summaryContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }, summaryCard: { flex: 1, borderRadius: 16, padding: 20, marginHorizontal: 5, borderWidth: 1, borderColor: theme.border }, summaryCardTitle: { fontSize: 14, color: theme.textSecondary, marginBottom: 5 }, summaryCardAmount: { fontSize: 20, fontWeight: 'bold' }, missionsCard: { backgroundColor: theme.card, borderWidth: 1, borderColor: theme.border, borderRadius: 16, padding: 20, marginBottom: 20 }, missionItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }, missionProgress: { color: theme.primary, fontWeight: '500' }, missionProgressContainer: { flexDirection: 'row' }, backButton: { width: 50 }, backButtonText: { fontSize: 30 }, formScrollContainer: { flexGrow: 1, justifyContent: 'space-between', padding: 20 }, formContainer: { padding: 20 }, formValueLabel: { fontSize: 16, color: theme.textSecondary, textAlign: 'center', marginBottom: 10 }, formValueInput: { fontSize: 40, fontWeight: 'bold', color: theme.primary, textAlign: 'center', marginBottom: 30, borderBottomWidth: 2, borderColor: theme.border, paddingBottom: 10 }, formRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.card, borderRadius: 12, padding: 15, marginBottom: 15, borderWidth: 1, borderColor: theme.border }, formLabel: { fontSize: 16, color: theme.text, marginRight: 10 }, formInput: { flex: 1, fontSize: 16, color: theme.textSecondary, textAlign: 'right' }, confirmButton: { marginTop: 20, marginBottom: 10 }, bottomButtonContainer: { padding: 20, borderTopWidth: 1, borderColor: theme.border, backgroundColor: theme.background }, modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' }, modalContent: { backgroundColor: theme.card, borderRadius: 16, padding: 20, width: '90%', maxHeight: '80%' }, modalTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 15, textAlign: 'center', color: theme.text }, modalOption: { paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: theme.border }, modalOptionText: { fontSize: 16, textAlign: 'center', color: theme.text }, modalCloseButton: { backgroundColor: theme.primary, borderRadius: 12, padding: 15, marginTop: 15 }, modalCloseButtonText: { color: '#fff', fontWeight: 'bold', textAlign: 'center', fontSize: 16 }, alertMessage: { fontSize: 16, color: theme.text, textAlign: 'center', marginBottom: 20},
